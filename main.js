...
const deleteBook = (bookId) => {
  ...
  // 書籍データの保存には、Firebase Storage（画像） とRealtime Database（書籍データ）を使っている
  //　イメージはそのままでいい。 
  // remove()を使う
  // 書籍データを保存するには firebase.database().ref(キー).push(値)
  firebase
    .database()
    .ref("books")
    .remove();
...
const loadBookshelfView = () => {
  ...
  booksRef.on('child_removed', (bookSnapshot) => {
    ...
    // 課題ゾーン
    // 書籍データの保存には、Firebase Storage とRealtime Databaseを使います。
